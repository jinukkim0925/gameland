	...
	public snake() {
		...
		addWindowListener(new WindowAdapter() {
			...
			public void windowClosed(WindowEvent e) {
				...
				run = false;
				snakecolor.run = false;
				snakecolor.tm.stop();
				snakecolor.sleep = 0;
				tm.stop();
				main_gameland.appendToPane(main_gameland.log, "스네이크 게임 종료!\n",new Color(16,122,34));
				main_gameland.ch = 0;
			}
		});
...
//		JOptionPane.showMessageDialog(null, "앗!");
		...
		if (bestscore >= 3) {
			...
			if (m == JOptionPane.YES_OPTION) {
				...
				logs.log = s;
				main_gameland.appendToPane(main_gameland.log, s+"님이 스네이크 게임에서 \n점수 : " + bestscore + "점 \n걸린시간 : " + ss.format(emilli - smilli)+"을 기록 하셨습니다.\n", new Color(79,247,135));
				snakecolor.run = false;
				snakecolor.tm.stop();
				
				dispose();
				main_gameland.ch = 0;
				for (int i = 0; i < main_gameland.listSnake.length; i++) {

					if (main_gameland.listSnake[i].getText().equals((i + 1) + ". ")) {
						main_gameland.listSnake[i]
								.setText((i + 1) + ". " + bestscore + "/" + ss.format(emilli - smilli) + "  (" + s + ")");
						break;
					} else {
						String sss[] = main_gameland.listSnake[i].getText().split(" ");
						String stt[] = sss[1].split("/");
						try {
							Date t1 = ss.parse(stt[1]);
							Date t2 = ss.parse(ss.format(emilli - smilli));
							if (Integer.parseInt(stt[0]) < bestscore) {
								
								String savest = main_gameland.listSnake[i].getText();
								for (int j = i; j < main_gameland.listSnake.length-1; j++) {
									String subsavest = main_gameland.listSnake[j+1].getText();
									main_gameland.listSnake[j+1].setText((j+2)+"."+savest.substring(2, savest.length()));
									savest = subsavest;
								}
								main_gameland.listSnake[i]
										.setText((i + 1) + ". " + bestscore + "/" + ss.format(emilli - smilli) + "  (" + s + ")");
								break;
							} else if (Integer.parseInt(stt[0]) == bestscore && t1.getTime() > t2.getTime()) {
								String savest = main_gameland.listSnake[i].getText();
								for (int j = i; j < main_gameland.listSnake.length-1; j++) {
									String subsavest = main_gameland.listSnake[j+1].getText();
									main_gameland.listSnake[j+1].setText((j+2)+"."+savest.substring(2, savest.length()));
