...
public class pac_man extends pac_man_frame {
	...
	int wherePac = 2, mouseSize = 0, drawPacX = 0, drawPacY = 0, draw = 0, sub = 3, where = 3, speed = 5;
	...
	public pac_man() {
		...
		this.addWindowListener(new WindowAdapter() {
			...
			public void windowClosing(WindowEvent e) {
				...
				where = 3;
				speed = 0;
				main_gameland.appendToPane(main_gameland.log, "팩맨 게임 종료!\n",new Color(244,49,28));
				main_gameland.ch = 0;
			}
		});
		
		pac_man_variable.goastSpeed = 10;
		speed = 5;
		pac_man_variable.run = true;
		run = true;
		turn = true;
		die = false;
		extrascore = false;
		...
		while (run) {
			try {
				...
				if (ch == 0) { // 클리어
					...
					pac_man_variable.run = false;
					gameTime.tm.stop();
					String s = JOptionPane.showInputDialog(this, "이름을 입력해 주세요.:");
					main_gameland.appendToPane(main_gameland.log, s+"님이 팩맨 게임에서 \n점수 : " + score + "점 \n걸린시간 : " + ss.format(emilli - smilli)+"을 기록 하셨습니다.\n", new Color(157,123,253));	
					dispose();
					reset();
					run = false;
					pac_man_variable.run = false;
					...
					where = 3;
					speed = 0;
					for (int i = 0; i < main_gameland.listPac.length; i++) {

						if (main_gameland.listPac[i].getText().equals((i + 1) + ". ")) {
							main_gameland.listPac[i]
									.setText((i + 1) + ". " + score + "/" + ss.format(emilli - smilli) + "  (" + s + ")");
							break;
						} else {
							String sss[] = main_gameland.listPac[i].getText().split(" ");
							String stt[] = sss[1].split("/");
							try {
								Date t1 = ss.parse(stt[1]);
								Date t2 = ss.parse(ss.format(emilli - smilli));
								if (Integer.parseInt(stt[0]) < score) {
									
									String savest = main_gameland.listPac[i].getText();
									for (int j = i; j < main_gameland.listPac.length-1; j++) {
										String subsavest = main_gameland.listPac[j+1].getText();
										main_gameland.listPac[j+1].setText((j+2)+"."+savest.substring(2, savest.length()));
										savest = subsavest;
									}
									main_gameland.listPac[i]
											.setText((i + 1) + ". " + score + "/" + ss.format(emilli - smilli) + "  (" + s + ")");
									break;
								} else if (Integer.parseInt(stt[0]) == score && t1.getTime() > t2.getTime()) {
									String savest = main_gameland.listPac[i].getText();
									for (int j = i; j < main_gameland.listPac.length-1; j++) {
										String subsavest = main_gameland.listPac[j+1].getText();
										main_gameland.listPac[j+1].setText((j+2)+"."+savest.substring(2, savest.length()));
